...
import {
  clampIntervalByPct,
  findNextRemovableVertex,
  findShapesByArcId,
  getAvgSegment2,
  quantizeArcs
} from '../src/paths/mapshaper-path-utils';
...
describe('mapshaper-shape-utils.js', function () {
  ...
  describe('#findNextRemovableVertex()', function () {
    it('Find index of largest non-infinite point', function () {
      ...
      id = findNextRemovableVertex(zz, Infinity, 0, 3);
      assert.equal(id, 1);
      zz = [Infinity, 0, 15, Infinity, Infinity, 5, -8, 2, 4, 5, Infinity];
      id = findNextRemovableVertex(zz, Infinity, 4, 10);
      assert.equal(id, 5);
      id = findNextRemovableVertex(zz, 4, 4, 10);
      assert.equal(id, 7);
    })
  })
  ...
  describe('findShapesByArcId()', function () {
    it('returns only shapes that contain one or more arc ids', function () {
      ...
      var arcIds = [0, 1];
      assert.deepEqual(findShapesByArcId(shapes, arcIds, 4), [1, 2, 3]);
    });
  })
  ...
  describe('quantizeArcs', function () {
    ...
    it('quantizeArcs() works', function() {
      ...
      // hi-res
      quantizeArcs(arcs, 9999); // multiple of 3, so original coords are preserved
