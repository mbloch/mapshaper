...
function geoJSONRoundTrip(fname) {
  ...
  var json = api.exportGeoJSONObject({arcs: arcs, shapes: topo.shapes, type: data.info.input_geometry_type, properties: data.properties})

  var data2 = api.importGeoJSON(json);
  var topo2 = api.buildTopology(data2);
  var arcs2 = new api.ArcDataset(topo2.arcs);
  var json2 = api.exportGeoJSONObject({arcs: arcs2, shapes: topo2.shapes, type: data2.info.input_geometry_type, properties: data2.properties})
