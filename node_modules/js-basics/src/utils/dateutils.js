...
DateString.prototype = (function() {
  ...
  p.moArr = moStr.split('|');
  p.fmtRxp = /%(a|A|b|B|d|m|y|Y|H|I|M|p)/g;
  ...
  p['d'] = p['m'] = p['I'] = p['H'] = p['M'] = p['y'] = '[0-9]{1,2}';
  p['Y'] = '[0-9]{4}';
...
DateString.prototype.parseDate = function(s) {
  ...
  if (!parts) {
    trace('[DateString.parseDate()] unable to parse:', s);
    return d;
  }
...
DateString.prototype.formatDatePart = function(d, code) {
  ...
  if (minLen > str.length) {
    str = TextUtils.leftPad(str, minLen, '0');
  }
  return str;
};
