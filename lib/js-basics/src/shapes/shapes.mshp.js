/** @requires events, core, browser, shapes, utf16, shapes.advanced */
...
function MshpParser(mshpStr, transform, types) {
  assert(Utils.isArray(types), "[MshpParser()] Requires an array of shape types.");
  ...
  if (!((metaData.shp_type == 5 || metaData.shp_type == 3) && metaData.topology)) {
    error("[MshpParser] Unable to parse; Parser currently only supports polygon or polyline data with topology. Found type:", metaData.shp_type, "topology?", metaData.topology);
    // return;
  }
  ...
  if (MshpParser.detailFactor) {
    scaleAdjustment *= MshpParser.detailFactor;
  }
  ...
  // trace(">>>> LINE SHAPES:", this._lineShapes)
...
MshpParser.prototype.parseVectors = function(bytes, transform, metaData) {
  ...
  for (var arcId = 0; arcId < numArcs; arcId++) {
    ...
    if (size > 0) {
      ...
      if (transform) {
        transform.transformXY(rawx, rawy, xy);
        x = xy.x;
        y = xy.y;
      }
      else {
