...
Dbf.initNumericField = function(arr, name) {
...
Dbf.initStringField = function(arr, name, encoding) {
  var info = {
    name: name,
    type: 'C',
    decimals: 0
...
Dbf.getFieldInfo = function(arr, name, encoding) {
  var type = this.discoverFieldType(arr, name),
      info;
  if (type == 'number') {
    info = Dbf.initNumericField(arr, name);
  } else if (type == 'string') {
    info = Dbf.initStringField(arr, name, encoding);
  } else {
