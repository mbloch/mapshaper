...
function Bundler() {
  ...
  var filePathIndex = {},   // paths of known js files indexed by name
      ...
      outFile = "out.js",   // set in bundleFile()
      mainFile;
  ...
  function getCmdOpts() {
    return Node.options({flags:"f"});
  }

  this.bundle = function() {
    var opts = getCmdOpts();
    if (opts._.length != 2) {
      trace("Usage: $ catty [-f] input output")
      process.exit();
    }
    this.bundleFile(opts._[0], opts._[1]);
  };
  ...
  this.bundleFile = function(src, dest, opts) {
    ...
    if (mainFile) {
      trace("Only one file can be bundled at a time; skipping:", src);
      return;
    }   

    // get absolute paths to the src and dest files
    mainFile = Node.absPath(src),
    outFile = Node.absPath(dest);

    assert(mainFile != outFile, "[bundleFile()] Source and destination paths are the same.");
    assert(Node.fileExists(mainFile), "[bundleFile()] Source file not found:", mainFile);
    var destInfo = Node.getFileInfo(outFile);
    assert(Node.dirExists(destInfo.directory), "[bundleFile()] Destination directory not found:", destInfo.directory);

    var key = indexFile(mainFile);
    monitorFile(key);
    bundle();
    trace("Wrote file:", outFile);
  ...
  function keyToPath(key) {
    assert(key in filePathIndex, "[keyToPath()] Unknown name:", key);
    return filePathIndex[key];
  }
  ...
  function monitorFile(key) {
    ...
    if (bundlerOpts.follow) {
      node.startMonitoring(function(node) {
        trace("Re-catting after change in", node.filename());
        bundle();        
      });
    }
    return true;
  ...
  this.addFile = function(path) {
    var absPath = Node.absPath(path);
    assert(Node.fileExists(absPath), "[addFile()] File not found:", path);
    indexFile(path);
  };
  ...
  function SourceFile(path) {  
    ...
    this.requiresFile = function(targNode, visited) {
      visited = visited || {};
      visited[this.name()] = true;
      var targName = targNode.name(),
          reqs = this.getDeps();
      ...
      for (var i=0; i<reqs.length; i++) {
        ...
        if (reqName in visited == false) {
          ...
          if (reqNode.requiresFile(targNode, visited)) {
            return true;
          }
        }
      }
    ...
    function update() {
      ...
      var newDeps = Utils.filter(_deps, monitorFile);
      return true;
    }

    function parseSource(js) {
      var requiresRxp = /(?:\/\*+|\/\/)\s*@require(?:s?)([ ,;_0-9A-Za-z.-]+)/g,
          fileRxp = /[_0-9a-z](?:[.-]?[_0-9a-z])*/ig,
          deps = [], match, match2;
    ...
    this.startMonitoring = function(callback) {
      ...
      fs.watch(_path, function(evt) {
        if (evt == "change") {
          ...
          // TODO: Avoid double-change on file write, e.g. by using a timeout
          if (update()) {
            callback(self);
          }
        } else if (evt == "renamed") {
          trace("Renamed:", _path, "TODO: handle this");
        } else {
