...
function Bundler() {
  ...
  this.bundleFile = function(src, dest, opts) {  
    ...
    var ifile = Node.resolvePathFromScript(src),
        ofile = Node.resolvePathFromScript(dest);
    bundleFile(ifile, ofile);
  };
  ...
  function monitorFile(key) {
    ...
    if (!path) {
      die("Unknown dependency, can't continue:", key, "-- need to add a src directory?");
    }
    ...
    if (bundlerOpts.follow) {
      node.startMonitoring(function(node) {
        trace("Re-catting -- change in", node.filename());
        bundle();        
      });
    }
    return true;
