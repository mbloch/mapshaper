...
//      var coords = s.getCoords(); // [x,y,x,y,...]
//      var zdata = s.getZ();  // [z,z,...]
//      var mdata = s.getM();  // [m,m,...] or null
//      var partSizes = s.getPartSizes(); // for types w/ parts
...
function ShpReader(src) {
  ...
  this.nextShape = function() {
    ...
    if (shape.byteLength <= 0) error("#nextShape() invalid record length, file may be correpted");
...
// Record methods
//   read() (all types)
//   getBounds(), getCoords()  (all but single point types)
//   getPartSizes() (polygon and polyline types)
//   getZBounds(), getZ() (Z types except POINTZ)
//   getMBounds(), getM(), hasM() (M and Z types, except POINT[MZ])
//
ShpReader.prototype.getRecordClass = function(type) {
  ...
  // Receive a BinArray set to the first byte of a shape
  //
  ...
  var multiCoordProto = {
    ...
    getPoints: function() {
      var coords = this.getCoords(),
          zz = hasZ ? this.getZ() : null,
          mm = hasM && this.hasM() ? this.getM() : null,
          points = [], p;
  ...
  var partsProto = {
    getPartSizes: function() {
      ...
      for (var i=0, n=this.partCount; i<n; i++) {
        ...
        if (partLen <= 0) error("ShapeRecord#getPartSizes() corrupted part");
        sizes.push(partLen);
        startId += partLen;
      }
      return sizes;
    ...
    read: function() {
      ...
      var parts = Utils.map(this.getPartSizes(), function(size) {
