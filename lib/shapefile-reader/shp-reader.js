...
ShpReader.prototype.forEachShape = function(callback) {
  ...
  while (this._bin.position(offs).bytesLeft() > 0) {
    ...
    if (i + 1 !== shape.id) error("ShpReader#forEachShape() id mismatch; expected:", i+1, "found:", shape.id)
...
ShpReader.prototype.readHeader = function(bin) {
