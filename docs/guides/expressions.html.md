...
If you need logic more complex than a ternary, define a helper function in an external file and load it with [`-include`](/docs/reference.html.md#-include):
...
`-run` accepts either a path to a [command file](/docs/essentials/command-line.html.md#command-files) or a string containing one or more curly-brace template expressions. Each `{...}` is evaluated as a JS expression and substituted into the resulting command string before Mapshaper parses it.
