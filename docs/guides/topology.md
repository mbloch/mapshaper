...
Snapping handles slightly offset pairs of vertices, but doesn't help much when adjacent polygons have small overlapping regions or gaps along their shared boundaries. The [`-clean`](/docs/reference.html#-clean) command repairs these by recomputing the polygon mosaic and snapping geometry that's nearly identical:
